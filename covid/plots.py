...
def rt(model, result):
    ...
    # Green Median < 0
    fig.add_trace(go.Scatter(
        x=x, y=result["median"].where(result["median"] <= 1),
...
def positives(result):
    ...
    fig.add_trace(go.Bar(x=result.index, y=result.positive,marker_color='rgba(150, 0, 0, .4)'))
    fig.show(config={'displayModeBar': False})
...
def tests(result):
    ...
    fig.add_trace(go.Bar(x=result.index, y=result.tests,marker_color='rgba(26, 118, 255, .4)'))
    fig.show(config={'displayModeBar': False})
